...
def build_factor_library(
    ...
    *,
    ...
    for dimension, specs in dimensions.items():
        for key, higher_is_better, _ in specs:
            values = {
                str(record["factor_id"]): (
                    _finite(record.get("metrics", {}).get(key))
                    if not (
                        current_protocols.get(
                            str(record.get("source_task_id", "legacy-ashare")),
                            current_protocol,
                        )
                    )
    ...
    for record in records:
        ...
        factor_current_protocol = current_protocols.get(source_task_id, current_protocol)
        protocol_stale = bool(
    ...
    factors.sort(key=lambda item: (-item["scores"]["overall"], item["source_iteration"]))
    ...
    return {
        ...
        "summary": {
            ...
            "ranking_method": "RETURN_25_ROBUSTNESS_30_RISK_15_EXECUTION_20_INFORMATION_10",
        },
    }
...
def _assign_similarity_clusters(factors: list[dict[str, Any]]) -> None:
    ...
    for cluster_number, group in enumerate(ordered, start=1):
        leader = max(group, key=lambda index: factors[index]["scores"]["overall"])
