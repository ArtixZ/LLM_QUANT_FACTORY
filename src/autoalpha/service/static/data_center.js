...
function renderSyncProgress(progress, running) {
  ...
  const label = active.adjustment === "raw_plus_adj_factor" ? "原始行情与复权因子" : active.adjustment === "qfq" ? "前复权" : "未复权";
  ...
  document.getElementById("syncPhaseList").replaceChildren(...tasks.map(task => {
    ...
    const phase = task.adjustment === "raw_plus_adj_factor" ? "原始行情 + 复权因子" : task.adjustment === "qfq" ? "qfq 研究源" : "未复权执行源";
...
async function startSync() {
  ...
  button.disabled = true;
  try { await api("/api/data-sync/start", { method: "POST", body: "{}" }); toast("增量同步已启动"); await loadDataCenter(); }
