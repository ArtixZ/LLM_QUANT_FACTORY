...
def test_trade_statement_serializes_fee_breakdown_and_security_name() -> None:
    ...
    market = pd.DataFrame({"symbol": ["AAPL"], "name": ["Apple Inc"]})

    statement = _trade_statement_rows(trades, market)

    assert statement[0]["trade_id"] == 1
    assert statement[0]["security_name"] == "Apple Inc"
    assert statement[0]["commission_usd"] == 5.0
    assert statement[0]["net_cash_flow_usd"] == -1005.01


def test_realistic_us_equity_preset_is_complete_and_tamper_evident() -> None:
    settings = MANUAL_BACKTEST_PRESETS[US_EQUITY_REALISTIC_WEEKLY_V1]["settings"]
    spec = ManualBacktestSpec(
        start_date=date(2020, 1, 2),
        end_date=date(2024, 12, 31),
        initial_cash_usd=1_000_000,
        backtest_preset=US_EQUITY_REALISTIC_WEEKLY_V1,
        **settings,
    )
...
def test_non_pit_proxy_preset_is_explicitly_research_only() -> None:
    settings = MANUAL_BACKTEST_PRESETS[US_EQUITY_ADJUSTED_PROXY_WEEKLY_V1]["settings"]
    spec = ManualBacktestSpec(
        start_date=date(2020, 1, 2),
        end_date=date(2024, 12, 31),
        initial_cash_usd=1_000_000,
